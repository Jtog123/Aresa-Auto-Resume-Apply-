...
describe("isValidProfile", () => {
    ...
    it("returns false when only name is set", () => {
        expect(isValidProfile({ ...createEmptyProfile(), name: "Alice" })).toBe(false);
    });
    it("returns false when only email is set", () => {
        expect(isValidProfile({ ...createEmptyProfile(), email: "a@b.com" })).toBe(false);
    });
    it("returns true when both name and email are set", () => {
        expect(isValidProfile({ ...createEmptyProfile(), name: "Alice", email: "a@b.com" })).toBe(true);
    });
});
